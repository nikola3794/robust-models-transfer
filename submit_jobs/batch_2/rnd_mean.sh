...
#BSUB -J "mean_stanford_cifar100"
#BSUB -R lca # workaround for the current wandb cluster bug

DATA_SET=stanford_cifar100
...
ARCH=vit_slope_param_deit_small_patch16_224
MIN_SLOPE=0.0
MAX_SLOPE=1.25
RND_ACT=False
OPTIMIZER=adamw
...
RND=$(( RANDOM % 999 ))
EXP_NAME=_only-slope-${DATA_SET}-${ARCH}-slope-${MIN_SLOPE}-${MAX_SLOPE}-rnd-${RND_ACT}-${OPTIMIZER}-${LR}-${RND}
